...
export function NeuralNoise({ color = [0.9, 0.2, 0.4], opacity = 0.95, speed = 0.001 }: { color?: number[]; opacity?: number; speed?: number }) {
  ...
  function getUniforms(gl: WebGLRenderingContext, program: WebGLProgram) {
    const uniformsDict: Record<string, WebGLUniformLocation | null> = {};
    const uniformCount = gl.getProgramParameter(program, gl.ACTIVE_UNIFORMS);
    for (let i = 0; i < uniformCount; i++) {
      const activeUniform = gl.getActiveUniform(program, i);
      if (activeUniform) {
        uniformsDict[activeUniform.name] = gl.getUniformLocation(program, activeUniform.name);
      }
    }
    return uniformsDict;
  }
